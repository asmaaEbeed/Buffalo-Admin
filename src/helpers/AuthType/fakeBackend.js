...
import {
  ...
  stock_balance_byStock,
  stock_balance_byMaterial
...
const fakeBackend = () => {
  ...
  // reports
  mock.onGet(url.GET_STOCK_BALANCE_BYSTOCK).reply(() => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (stock_balance_byStock) {
          // Passing fake JSON data as response
          resolve([200, stock_balance_byStock]);
        } else {
          reject([400, "Cannot get products"]);
        }
      });
  ...
  mock.onGet(url.Get_STOCK_BALANCE_BYMATERIAL).reply(() => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (stock_balance_byMaterial) {
          // Passing fake JSON data as response
          resolve([200, stock_balance_byMaterial]);
        } else {
          reject([400, "Cannot get products"]);
        }
      });
