...
//////// Buffalo Login ////////////
const postLogin = async (data, config = {}) => {
  try {
    console.log(data)
    return await axiosInstance.post("/Login", data, { ...config }).then((response) => response)
  } catch({response}){
    return response;
  }
}
...
//reports
const getStockBalanceByStock = () => get(url.GET_STOCK_BALANCE_BYSTOCK);

const getStockBalanceByMaterial = () => get(url.Get_STOCK_BALANCE_BYMATERIAL)
...
export {
  ...
  postLogin,
  getStockBalanceByStock,
  getStockBalanceByMaterial
};
